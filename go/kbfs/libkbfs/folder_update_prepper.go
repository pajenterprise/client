...
func (fup *folderUpdatePrepper) unembedBlockChanges(
	...
	ptr := data.BlockPointer{
		...
		data.DataVer: fup.config.DataVersion(),
		DirectType:   DirectBlock,
		Context: kbfsblock.MakeFirstContext(
			chargedTo, keybase1.BlockType_MD),
	}
	file := data.Path{fup.folderBranch,
		[]data.PathNode{{ptr, fmt.Sprintf("<MD rev %d>", md.Revision())}}}
...
func (fup *folderUpdatePrepper) prepUpdateForPath(
	...
	currName := name
	newPath := data.Path{
		data.FolderBranch: dir.FolderBranch,
		data.Path:         make([]data.PathNode, 0, len(dir.path)),
	}
	...
	for len(newPath.path) < len(dir.path)+1 {
		...
		newPath.path = append([]data.PathNode{{info.BlockPointer, currName}},
			newPath.path...)
...
func (fup *folderUpdatePrepper) setChildrenNodes(
	...
	for _, name := range names {
		...
		childPath := data.Path{
			data.FolderBranch: p.FolderBranch,
			data.Path:         make([]data.PathNode, indexInPath+2),
		}
