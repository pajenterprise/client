	...
	return runUnlessCanceled(ctx, func() error {
		if md.TlfID() != fbo.id() {
			return WrongOpsError{
				fbo.folderBranch, data.FolderBranch{md.TlfID(), data.MasterBranch}}
		}
...
func (fbo *folderBranchOps) SetInitialHeadToNew(
	...
	return runUnlessCanceled(ctx, func() error {
		...
		fb := data.FolderBranch{rmd.TlfID(), data.MasterBranch}
		if fb != fbo.folderBranch {
			return WrongOpsError{fbo.folderBranch, fb}
		}
	...
	de = data.DirEntry{
		data.BlockInfo: data.BlockInfo{
			data.BlockPointer: data.BlockPointer{
				ID:           id,
				data.DataVer: FirstValidDataVer,
			},
		},
		data.EntryInfo: data.EntryInfo{
			Type:  data.Dir,
...
func (fbo *folderBranchOps) makeFakeFileEntry(
	...
	de = data.DirEntry{
		...
		data.EntryInfo: EntryInfoFromFileInfo(fi),
	}
	if de.Type == data.Sym {
		target, err := fs.Readlink(name)
		if err != nil {
...
func (fbo *folderBranchOps) createEntryLocked(
	...
	if err == nil {
		return nil, data.DirEntry{}, NameExistsError{name}
	...
	newPtr := data.BlockPointer{
		ID:           newID,
		KeyGen:       md.LatestKeyGeneration(),
		data.DataVer: fbo.config.DataVersion(),
...
func (fbo *folderBranchOps) CreateDir(
	...
	fbo.log.CDebugf(ctx, "CreateDir %s %s", getNodeIDStr(dir), data.Path)
	defer func() {
		fbo.deferLog.CDebugf(ctx, "CreateDir %s %s done: %v %+v",
			getNodeIDStr(dir), data.Path, getNodeIDStr(n), err)
	}()
	...
	err = fbo.doMDWriteWithRetryUnlessCanceled(ctx,
		func(lState *kbfssync.LockState) error {
			node, de, err :=
				fbo.createEntryLocked(ctx, lState, dir, data.Path, data.Dir, NoExcl)
...
func (fbo *folderBranchOps) CreateFile(
	...
	fbo.log.CDebugf(ctx, "CreateFile %s %s isExec=%v Excl=%s",
		getNodeIDStr(dir), data.Path, isExec, excl)
	defer func() {
		fbo.deferLog.CDebugf(ctx,
	...
	err = fbo.doMDWriteWithRetryUnlessCanceled(ctx,
		func(lState *kbfssync.LockState) error {
			...
			// race when the Create is canceled.
			node, de, err :=
				fbo.createEntryLocked(ctx, lState, dir, data.Path, entryType, excl)
			retNode = node
			retEntryInfo = de.EntryInfo
			return err
		})
...
func (fbo *folderBranchOps) createLinkLocked(
	...
	if err == nil {
		return data.DirEntry{}, NameExistsError{fromName}
	} else if _, notExists := errors.Cause(err).(idutil.NoSuchNameError); !notExists {
		return data.DirEntry{}, err
	}
...
func (fbo *folderBranchOps) RequestRekey(_ context.Context, tlf tlf.ID) {
	...
	fb := data.FolderBranch{tlf, data.MasterBranch}
	if fb != fbo.folderBranch {
...
func (fbo *folderBranchOps) MigrateToImplicitTeam(
	...
	fb := data.FolderBranch{id, data.MasterBranch}
	if fb != fbo.folderBranch {
		// TODO: log instead of panic?
		panic(WrongOpsError{fbo.folderBranch, fb})
	}
...
func (fbo *folderBranchOps) GetSyncConfig(
	...
	if tlfID != fbo.id() || fbo.branch() != data.MasterBranch {
		return keybase1.FolderSyncConfig{}, WrongOpsError{
			fbo.folderBranch, data.FolderBranch{tlfID, data.MasterBranch}}
	}
