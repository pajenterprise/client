...
func setupNodeCache(t *testing.T, id tlf.ID, branch data.BranchName, flat bool) (
	...
	childPath1 = []data.PathNode{
		{
			data.BlockPointer: parentPtr,
			Name:              parentName,
		},
		{
			data.BlockPointer: childPtr1,
			Name:              childName1,
		},
	}
	if flat {
...
func checkNodeCachePath(t *testing.T, id tlf.ID, branch data.BranchName,
	...
	if len(data.Path.path) != len(expectedPath) {
		t.Errorf("Bad path length: %v vs %v", len(data.Path.path), len(expectedPath))
	}

	for i, n := range expectedPath {
		if data.Path.path[i] != n {
			t.Errorf("Bad node on path, index %d: %v vs %v", i, data.Path.path[i], n)
		}
	}
	if data.Path.Tlf != id {
		t.Errorf("Wrong top dir: %v vs %v", data.Path.Tlf, id)
	}
	if data.Path.Branch != data.BranchName(branch) {
		t.Errorf("Wrong branch: %s vs %s", data.Path.Branch, branch)
	}
}
...
func TestNodeCacheWrapChild(t *testing.T) {
	ncs := newNodeCacheStandard(
		FolderBranch{tlf.FakeID(0, tlf.Private), data.MasterBranch})
